...
contract BrewlabsFarm is Ownable, ReentrancyGuard {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accTokenPerShare = accTokenPerShare.add(
                brewsReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accTokenPerShare).div(1e12).sub(user.rewardDebt);
    }

    function pendingReflections(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 _accReflectionPerPoint = accReflectionPerPoint.add(
                reflectionAmt.mul(1e12).div(totalAllocPoint)
            );
            ...
            accReflectionPerShare = pool.accReflectionPerShare.add(
                pool
                    .allocPoint
                    .mul(
                        _accReflectionPerPoint.sub(pool.lastReflectionPerPoint)
                    )
                    .div(lpSupply)
            );
        }
        return
            user.amount.mul(accReflectionPerShare).div(1e12).sub(
                user.reflectionDebt
            );
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        accReflectionPerPoint = accReflectionPerPoint.add(
            reflectionAmt.mul(1e12).div(totalAllocPoint)
        );
        pool.accReflectionPerShare = pool.accReflectionPerShare.add(
            pool
                .allocPoint
                .mul(accReflectionPerPoint.sub(pool.lastReflectionPerPoint))
                .div(lpSupply)
        );
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            uint256 pendingReflection = user
                .amount
                .mul(pool.accReflectionPerShare)
                .div(1e12)
        ...
        user.reflectionDebt = user.amount.mul(pool.accReflectionPerShare).div(
            1e12
        );
    ...
    function recoverWrongToken(address _token) external onlyOwner {
        require(
            _token != address(brews) && _token != address(reflectionToken),
