...
import "./oz/contracts/math/SafeMath.sol";
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    uint256 public constant MASTERCHEF_CAKE_PER_BLOCK = 40 * 1e10;
    ...
    function cakePerBlock(bool _isRegular)
        public
        view
        returns (uint256 amount)
    {
        if (_isRegular) {
            ...
            amount = MASTERCHEF_CAKE_PER_BLOCK.mul(cakeRateToSpecialFarm).div(
                CAKE_RATE_TOTAL_PRECISION
            );
        }
    }
    ...
    function cakePerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_CAKE_PER_BLOCK.mul(cakeRateToBurn).div(
            CAKE_RATE_TOTAL_PRECISION
        );
    }
