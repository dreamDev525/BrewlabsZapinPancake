    ...
    DividendPayingTokenOptionalInterface
{
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            if (
                ...
                return
                    swapBNBForTokensAndWithdrawDividend(
                        user,
                        _withdrawableDividend
                    );
            }
    ...
    function swapBNBForTokensAndWithdrawDividend(
        address holder,
        uint256 bnbAmount
            ...
            )
        {
            ...
            withdrawnDividends[holder] = withdrawnDividends[holder].sub(
                bnbAmount
            );
            return 0;
        }
...
contract Brewlabs is Ownable, ERC20 {
    ...
    CustomTaxPeriod private _launch1 =
        CustomTaxPeriod("launch1", 3, 0, 3, 100, 3, 0, 1, 0, 1, 0, 1, 0, 4, 0);
    CustomTaxPeriod private _launch2 =
        CustomTaxPeriod(
            "launch2",
            0,
            3600,
    ...
    CustomTaxPeriod private _launch3 =
        CustomTaxPeriod(
            "launch3",
            0,
            82800,
    ...
    // Base taxes
    CustomTaxPeriod private _default =
        CustomTaxPeriod("default", 0, 0, 3, 3, 3, 4, 1, 1, 1, 2, 1, 1, 4, 4);
    CustomTaxPeriod private _base =
        CustomTaxPeriod("base", 0, 0, 3, 3, 3, 4, 1, 1, 1, 2, 1, 1, 4, 4);

    // Golden Hour taxes
    uint256 private _goldenHourStartTimestamp;
    CustomTaxPeriod private _golden1 =
        CustomTaxPeriod(
            "golden1",
            0,
            3600,
    ...
    CustomTaxPeriod private _golden2 =
        CustomTaxPeriod(
            "golden2",
            0,
            3600,
    ...
    event MintOnEmission(
        address mintAddress,
        uint256 mintAmount,
        uint256 blockTracker,
        uint256 blockNumber
    );
    ...
    function cancelLaunch() public onlyOwner {
        require(this.isInLaunch(), "Brewlabs: Launch is not set");
    ...
    function activateMintOnBlockEmission(
        address _mintAddress,
        uint256 _mintAmount
    ) public onlyOwner {
        require(!mintOnBlockEmission, "Brewlabs: Mint is already set activate");
    ...
    function deactivateMintOnBlockEmission() public onlyOwner {
        require(
            mintOnBlockEmission,
            "Brewlabs: Mint is already set deactivate"
        );
    ...
    function setGoldenHour() public onlyOwner {
        require(!this.isInGoldenHour(), "Brewlabs: Golden Hour is already set");
        require(isTradingEnabled, "Brewlabs: Trading must be enabled first");
        require(!this.isInLaunch(), "Brewlabs: Must not be in launch period");
        emit GoldenHourChange(true, false);
        _goldenHourStartTimestamp = _getNow();
    }

    function cancelGoldenHour() public onlyOwner {
        require(this.isInGoldenHour(), "Brewlabs: Golden Hour is not set");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "Brewlabs: The dividend tracker already has that address"
        );
        BrewlabsDividendTracker newDividendTracker = BrewlabsDividendTracker(
            payable(newAddress)
        );
        require(
            newDividendTracker.owner() == address(this),
            "Brewlabs: The new dividend tracker must be owned by the Brewlabs token contract"
        );
    ...
    function blockAccount(address account) public onlyOwner {
        ...
        require(!_isBlocked[account], "Brewlabs: Account is already blocked");
        if (_isLaunched) {
            require(
                currentTimestamp.sub(_launchStartTimestamp) < _blockedTimeLimit,
                "Brewlabs: Time to block accounts has expired"
            );
        }
    ...
    function unblockAccount(address account) public onlyOwner {
        require(_isBlocked[account], "Brewlabs: Account is not blcoked");
    ...
    function setWallets(
        ...
        if (liquidityWallet != newLiquidityWallet) {
            require(
                ...
                "Brewlabs: The liquidityWallet cannot be 0"
            );
            emit WalletChange(
                "liquidityWallet",
                newLiquidityWallet,
        ...
        if (marketingWallet != newMarketingWallet) {
            require(
                ...
                "Brewlabs: The marketingWallet cannot be 0"
            );
            emit WalletChange(
                "marketingWallet",
                newMarketingWallet,
        ...
        if (devWallet != newDevWallet) {
            require(
                newDevWallet != address(0),
                "Brewlabs: The devWallet cannot be 0"
            );
        ...
        if (buyBackWallet != newBuyBackWallet) {
            require(
                ...
                "Brewlabs: The buyBackWallet cannot be 0"
            );
    ...
    function setStakingAddress(address newStakingAddress) public onlyOwner {
        require(
            newStakingAddress != address(0),
            "Brewlabs: The stakingAddress cannot be 0"
        );
    ...
    function resetAllFees() public onlyOwner {
        _setCustomBuyTaxPeriod(
            _base,
            _default.liquidityFeeOnBuy,
            _default.marketingFeeOnBuy,
            _default.devFeeOnBuy,
            _default.buyBackFeeOnBuy,
            _default.stakingFeeOnBuy,
        ...
        _setCustomSellTaxPeriod(
            _base,
            _default.liquidityFeeOnSell,
            _default.marketingFeeOnSell,
            _default.devFeeOnSell,
            _default.buyBackFeeOnSell,
            _default.stakingFeeOnSell,
    ...
    // Golden Hour 1 Fees
    function setGoldenHour1BuyFees(
        uint256 _liquidityFeeOnBuy,
        uint256 _marketingFeeOnBuy,
        uint256 _devFeeOnBuy,
        uint256 _buyBackFeeOnBuy,
    ...
    function setGoldenHour1SellFees(
        uint256 _liquidityFeeOnSell,
        uint256 _marketingFeeOnSell,
        uint256 _devFeeOnSell,
        uint256 _buyBackFeeOnSell,
    ...
    // Golden Hour 2 Fees
    function setGoldenHour2BuyFees(
        uint256 _liquidityFeeOnBuy,
        uint256 _marketingFeeOnBuy,
        uint256 _devFeeOnBuy,
        uint256 _buyBackFeeOnBuy,
    ...
    function setGoldenHour2SellFees(
        uint256 _liquidityFeeOnSell,
        uint256 _marketingFeeOnSell,
        uint256 _devFeeOnSell,
        uint256 _buyBackFeeOnSell,
    ...
    function setGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "Brewlabs: Cannot update gasForProcessing to same value"
        );
    ...
    function setMaxTransactionAmount(uint256 newValue) public onlyOwner {
        require(
            ...
            "Brewlabs: Cannot update maxTxAmount to same value"
        );
    ...
    function setDividendToken(address newDividendToken) external onlyOwner {
        require(
            newDividendToken != dividendToken,
            "Brewlabs: Cannot update dividend token to same value"
        );
        require(
            newDividendToken != address(0),
            "Brewlabs: The dividend token cannot be 0"
        );
        require(
            newDividendToken != address(this),
            "Brewlabs: The dividend token cannot be set to the current contract"
        );
    ...
    function claimBNBOverflow(uint256 amount) external onlyOwner {
        require(
            ...
            "Brewlabs: Cannot send more than contract balance"
        );
    ...
    function timeSinceLastGoldenHour() external view returns (uint256) {
        uint256 currentTimestamp = !isTradingEnabled &&
            _tradingPausedTimestamp > _goldenHourStartTimestamp
            ? _tradingPausedTimestamp
            : _getNow();
    ...
    function isInGoldenHour() external view returns (bool) {
        uint256 currentTimestamp = !isTradingEnabled &&
            ...
            ? _tradingPausedTimestamp
            : _getNow();
        uint256 totalGoldenTime = _golden1.timeInPeriod.add(
            _golden2.timeInPeriod
        );
    ...
    function isInLaunch() external view returns (bool) {
        uint256 currentTimestamp = !isTradingEnabled &&
            _tradingPausedTimestamp > _launchStartTimestamp
            ? _tradingPausedTimestamp
            : _getNow();
        uint256 totalLaunchTime = _launch1
            .timeInPeriod
            .add(_launch2.timeInPeriod)
            .add(_launch3.timeInPeriod);
        if (
            _isLaunched &&
            (currentTimestamp.sub(_launchStartTimestamp) < totalLaunchTime ||
                block.number.sub(_launchBlockNumber) < _launch1.blocksInPeriod)
        ) {
            return true;
        } else {
            return false;
    ...
    {
        return (
            _golden1.liquidityFeeOnBuy,
            _golden1.marketingFeeOnBuy,
            _golden1.devFeeOnBuy,
            _golden1.buyBackFeeOnBuy,
            _golden1.stakingFeeOnBuy,
            _golden1.holdersFeeOnBuy
        );
    }

    function getGolden1SellFees()
        external
        view
        returns (
            uint256,
    ...
    {
        return (
            _golden1.liquidityFeeOnSell,
            _golden1.marketingFeeOnSell,
            _golden1.devFeeOnSell,
            _golden1.buyBackFeeOnSell,
            _golden1.stakingFeeOnSell,
            _golden1.holdersFeeOnSell
        );
    }

    function getGolden2BuyFees()
        external
        view
        returns (
            uint256,
    ...
    {
        return (
            _golden2.liquidityFeeOnBuy,
            _golden2.marketingFeeOnBuy,
            _golden2.devFeeOnBuy,
            _golden2.buyBackFeeOnBuy,
            _golden2.stakingFeeOnBuy,
            _golden2.holdersFeeOnBuy
        );
    }

    function getGolden2SellFees()
        external
        view
        returns (
            uint256,
    ...
    {
        return (
            _golden2.liquidityFeeOnSell,
            _golden2.marketingFeeOnSell,
            _golden2.devFeeOnSell,
            _golden2.buyBackFeeOnSell,
            _golden2.stakingFeeOnSell,
            _golden2.holdersFeeOnSell
        );
    }
    ...
    function _transfer(
        ...
        if (
            ...
            require(
                isTradingEnabled,
                "Brewlabs: Trading is currently disabled."
            );
            require(!_isBlocked[to], "Brewlabs: Account is blocked");
            require(!_isBlocked[from], "Brewlabs: Account is blocked");
            if (
                _isInLaunch &&
                currentTimestamp.sub(_launchStartTimestamp) <= 300 &&
                isBuyFromLp
            ) {
                require(
                    currentTimestamp.sub(_buyTimesInLaunch[to]) > 60,
                    "Brewlabs: Cannot buy more than once per min in first 5min of launch"
                );
            }
            if (
                ...
                require(
                    amount <= maxTxAmount,
                    "Brewlabs: Buy amount exceeds the maxTxBuyAmount."
                );
            }
            if (!_isExcludedFromMaxWalletLimit[to]) {
                require(
                    balanceOf(to).add(amount) <= maxWalletAmount,
                    "Brewlabs: Expected wallet amount exceeds the maxWalletAmount."
                );
            }
        }
    ...
    function _mintOnBlockEmission() private {
        if (mintOnBlockEmission && _blockTracker < block.number) {
            uint256 _mintAmount = mintAmount.mul(
                block.number.sub(_blockTracker)
            );
    ...
    function _adjustTaxes(bool isBuyFromLp, bool isSelltoLp) private {
        ...
        uint256 timeSinceGolden = currentTimestamp.sub(
            _goldenHourStartTimestamp
        );
        _liquidityFee = 0;
        _marketingFee = 0;
        _devFee = 0;
        ...
        if (isBuyFromLp) {
            ...
            if (
                _isLaunched &&
                timeSinceLaunch <= _launch2.timeInPeriod &&
            ...
            if (
                timeSinceGolden > _golden1.timeInPeriod &&
                timeSinceGolden <=
                _golden1.timeInPeriod.add(_golden2.timeInPeriod)
            ) {
    ...
    function _swapAndLiquify() private {
        ...
        uint256 amountToLiquify = contractBalance
            .mul(_liquidityFee)
            .div(_totalFee)
            .div(2);
        uint256 amountForStaking = contractBalance.mul(_stakingFee).div(
            _totalFee
        );
        uint256 amountToSwap = contractBalance.sub(
            (amountToLiquify.add(amountForStaking))
        );

        _swapTokensForBNB(amountToSwap);

        uint256 bnbBalanceAfterSwap = address(this).balance.sub(
            initialBNBBalance
        );
        uint256 totalBNBFee = _totalFee.sub(_liquidityFee.div(2));
        uint256 amountBNBLiquidity = bnbBalanceAfterSwap
            .mul(_liquidityFee)
            .div(totalBNBFee)
            .div(2);
        uint256 amountBNBMarketing = bnbBalanceAfterSwap.mul(_marketingFee).div(
            totalBNBFee
        );
        uint256 amountBNBBuyBack = bnbBalanceAfterSwap.mul(_buyBackFee).div(
            totalBNBFee
        );
        uint256 amountBNBDev = bnbBalanceAfterSwap.mul(_devFee).div(
            totalBNBFee
        );
        uint256 amountBNBHolders = bnbBalanceAfterSwap.sub(
            (
                amountBNBLiquidity
                    .add(amountBNBMarketing)
                    .add(amountBNBBuyBack)
                    .add(amountBNBDev)
            )
        );
        ...
        bool stakingSuccess = IERC20(address(this)).transfer(
        ...
        (bool dividendSuccess, ) = address(dividendTracker).call{
            value: amountBNBHolders
        }("");
        if (dividendSuccess) {
...
contract BrewlabsDividendTracker is DividendPayingToken {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Brewlabs_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "Brewlabs_Dividend_Tracker: Cannot update claimWait to same value"
        );
