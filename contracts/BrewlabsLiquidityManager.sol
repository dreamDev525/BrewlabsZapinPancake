...
contract BrewlabsLiquidityManager is Ownable, ReentrancyGuard {
        ...
        )
    {
        ...
        uint256 token0Amt = IERC20(token0).balanceOf(address(this)).sub(
            beforeAmt
        );
        ...
        uint256 token1Amt = IERC20(token1).balanceOf(address(this)).sub(
            beforeAmt
        );
        ...
        )
    {
        ...
        uint256 tokenAmt = IERC20(token).balanceOf(address(this)).sub(
            beforeAmt
        );
        ...
        )
    {
        ...
        return
            IUniRouter02(uniRouterAddress).addLiquidityETH{value: _ethAmt}(
                token,
                _amount,
    ...
    function removeLiquidity(
        ...
        IERC20(token0).safeTransfer(
            msg.sender,
            amountA.mul(10000 - fee).div(10000)
        );
        IERC20(token1).safeTransfer(
            msg.sender,
            amountB.mul(10000 - fee).div(10000)
        );
        ...
        returns (uint256 amountToken, uint256 amountETH)
    {
        ...
        IERC20(token).safeTransfer(
            msg.sender,
            amountToken.mul(10000 - fee).div(10000)
        );
